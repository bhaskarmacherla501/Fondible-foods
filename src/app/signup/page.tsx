...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!data.success) { toast.error(data.error ?? 'Failed to create account'); return }

      const signInRes = await signIn('credentials', { email, password, redirect: false })
      if (signInRes?.error) { toast.error('Account created — please sign in'); router.push('/login'); return }

      toast.success('Welcome to Fondible!')
    ...
    <div className="page-container py-20">
      <div className="max-w-md mx-auto">
        <div className="text-center mb-8">
          <span className="section-label">Join Fondible</span>
          <h1 className="section-title text-brown mt-2">Create Account</h1>
        </div>
