...
export async function Footer() {
  ...
  const waNumber = process.env.NEXT_PUBLIC_WA_NUMBER ?? settings.whatsappNumber
      ...
      <div className="page-container">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-10 mb-12">
          ...
          <div className="col-span-2 md:col-span-1 lg:col-span-1">
            ...
            <a href={`https://wa.me/${waNumber}?text=Hi%20Fondible!%20I%20need%20help%20with%20something.`} target="_blank" rel="noopener noreferrer"
              ...
              <div className="flex items-center gap-2 mt-3">
                {settings.instagramUrl && (
                  <a href={settings.instagramUrl} target="_blank" rel="noopener noreferrer"
                    className="w-8 h-8 flex items-center justify-center rounded-full bg-cream/5 border border-cream/8 text-cream/60 hover:text-gold hover:border-gold/40 transition-colors">
          ...
          {Object.entries(FOOTER_LINKS).map(([section, links]) => (
            <div key={section}>
              <h4 className="footer-heading uppercase text-gold mb-4">
              ...
              <ul className="space-y-2.5">
                {links.map(link => (
                  <li key={link.href}>
                    <Link href={link.href} className="footer-link text-cream/60 hover:text-gold transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="border-t border-cream/8 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-cream/40">© 2025 Fondible · Made with ❤️ in India</p>
